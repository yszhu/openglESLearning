package com.example.admin.vertexbufferobject;
...
public class VBORenderer implements GLSurfaceView.Renderer
{
    ...
    private void drawPrimitiveWithoutVBOs()
    {
        ...
        //maybe it mean to disable(or free) vertex buffer object
        //note 3.1 by yszhu
    ...
    private void drawPrimitiveWithVBOs()
    {
        ...
        // when data be transported to shader program can't use
        // array type just like float[] ,index[] ect ,it must be the buffer  ,it's easy
        //to get buffer from array like:
//        mIndices = ByteBuffer.allocateDirect ( mIndicesData.length * 2 )
//                .order ( ByteOrder.nativeOrder() ).asShortBuffer();
//        mIndices.put ( mIndicesData ).position ( 0 );
...
//                GLES30.GL_FLOAT, false, vtxStride, mVertices );
        ...
        // if use array ,the offset must be set by calling position set function of buffer
        // like :
        // mVertices.position ( 0 );
        // mVertices.position ( VERTEX_POS_SIZE );
